...
// Enable Helmet to set security headers (CSP configured loosely to support serve-from-db uploads & assets)
app.use(helmet({
  contentSecurityPolicy: false
}));

// Restrict CORS origins (Can be limited to exact domains in production via whitelist environment configs)
app.use(cors({
  origin: process.env.ALLOWED_ORIGINS ? process.env.ALLOWED_ORIGINS.split(',') : '*',
  credentials: true
}));

// Limit request payload sizes to protect against memory depletion DOS attacks
